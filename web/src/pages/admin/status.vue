        ...
        <div class="card">
                ...
                <div class="row justify-content-end">
                    <div class="col-auto">
                        <button class="btn btn-primary btn-lg" type="button" data-bs-toggle="modal"
                            data-original-title="test" data-bs-target="#exampleModal">Create New Status</button>
                    </div>
                </div>
                <div class="modal fade" id="exampleModal" tabindex="-1" status="dialog"
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered display" id="basic-1">
                        ...
                        <tbody>
                            <tr v-for="status in statuses" :key="status.id">
                                ...
                                <!-- <td v-if="status.isActive == true">Active </td>
                                <td v-else-if="status.isActive == false">Disabled</td> -->
                                <!-- <td>{{ status.isActive }}</td> -->
                                <td>
                                    <!-- Edit Icon -->
                                    <a class="text-success" data-bs-toggle="modal" data-bs-target="#editModal"
                                        @click="setItem(status)">
                    ...
                    <!-- Edit Modal -->
                    <div class="modal fade" id="editModal" tabindex="-1" status="dialog"
                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" status="document">
                            <div class="modal-content">
                                <div class="modal-header">
                    ...
                    <div class="modal fade" id="deleteModal" tabindex="-1" status="dialog"
                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" status="document">
                            <div class="modal-content">
                                <div class="modal-header">
...
export default {
    ...
    methods: {
        ...
        async fetchStatuses() {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/admin/status/fetch-all', {
        ...
        async createData() {
            ...
            axios.post('http://127.0.0.1:8000/api/admin/status/create', newData, {
                headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('HCA_TOKEN') }
            })
                .then(response => {
                    console.log('success')
                    ...
                    window.location.href = 'http://localhost:8080/admin/status'
        ...
        async updateData() {
            ...
            axios.post('http://127.0.0.1:8000/api/admin/status/update', newData, {
                headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('HCA_TOKEN') }
            })
                .then(response => {
                    console.log('success')
        ...
        async deleteData() {
            ...
            axios.post('http://127.0.0.1:8000/api/admin/status/deactivate', itemData, {
                headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('HCA_TOKEN') }
            })
                .then(response => {
                    console.log('success')
        ...
        num_pages() {
            return Math.ceil(this.statuses.length / this.elementsPerPage);
        },
        change_page(page) {
